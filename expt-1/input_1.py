...
class PseudoInput(snt.AbstractModule):
    def __init__(self, batch_size, z_dim = 1, min_x = -8., max_x = 8., 
                 min_y = 0., max_y = 0.30, debug = False, name = "pseudo_input"):
    ...
    def _pdf(self, x):
        y = 0.50 / tf.sqrt((2*self._pi)) * tf.exp(-tf.square(x - 2.0) / 2) + \
            0.50 / tf.sqrt((2*self._pi)) * tf.exp(-tf.square(x + 2.0) / 2)
